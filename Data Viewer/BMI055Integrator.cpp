 ...
 */
 ...
 /*Modified for usage in LibPSVR Data Viewer*/
...
MadgwickAHRS    BMI055Integrator::fusion = MadgwickAHRS(glm::quat(1.0f, 0.0f, 0.0f, 0.0f));
glm::quat       BMI055Integrator::ZeroPose = glm::quat(1.0f, 0.0f, 0.0f, 0.0f);
...
glm::quat BMI055Integrator::Parse(void *data) {
	...
	auto linearAcceleration1 = aRes * glm::vec3(frame->s.data[0].accel.x, frame->s.data[0].accel.y, frame->s.data[0].accel.z);
	auto angularAcceleration1 = gRes * glm::vec3(frame->s.data[0].gyro.yaw, frame->s.data[0].gyro.pitch, frame->s.data[0].gyro.roll);
	Integrate(linearAcceleration1, angularAcceleration1, frame->s.data[0].timestamp);

	auto linearAcceleration2 = aRes * glm::vec3(frame->s.data[1].accel.x, frame->s.data[1].accel.y, frame->s.data[1].accel.z);
	auto angularAcceleration2 = gRes * glm::vec3(frame->s.data[1].gyro.yaw, frame->s.data[1].gyro.pitch, frame->s.data[1].gyro.roll);

	return Integrate(linearAcceleration2, angularAcceleration2, frame->s.data[1].timestamp);
}
...
float BMI055Integrator::GetGres(Gscale Scale) {
	...
	case Gscale::GFS_125DPS:
		return (float)(0.00381f * (glm::pi<float>() / 180.0f)); //return 124.87 / (32768.0 * 4); // per data sheet, not exactly 125!?
	case Gscale::GFS_250DPS:
		return (float)(0.007622f * (glm::pi<float>() / 180.0f)); //1.0 / 262.4; //return 249.75 / 32768.0;
	case Gscale::GFS_500DPS:
		return (float)(0.01524f * (glm::pi<float>() / 180.0f));//1.0 / 262.4; //return 499.5 / 32768.0;
	case Gscale::GFS_1000DPS:
		return (float)(0.03048f * (glm::pi<float>() / 180.0f)); //1.0 / 262.4; //return 999.0 / 32768.0;
	case Gscale::GFS_2000DPS:
		return (float)(0.06097f * (glm::pi<float>() / 180.0f));//1.0 / 262.4; //return 1998.0 / 32768.0;
	}
	return 0;
}
...
glm::quat BMI055Integrator::Integrate(glm::vec3 linearAcceleration, glm::vec3 angularAcceleration, uint32_t Timestamp) {
	if (samplesLeft > 0) {
		...
		fusion = MadgwickAHRS(glm::quat(1.0f, 0.0f, 0.0f, 0.0f));
		return glm::quat(1.0f, 0.0f, 0.0f, 0.0f);
		...
		fusion.Update(angularAcceleration.x, angularAcceleration.y, angularAcceleration.z,
					  ...
					  0.035f, interval);
		prevTimestamp = Timestamp;
		ZeroPose = glm::quat(1.0f, 0.0f, 0.0f, 0.0f) * glm::inverse(fusion.Quaternion);
...
glm::vec3 BMI055Integrator::ToEuler(glm::quat *Q) {
	...
	pitchYawRoll.x = -(float)glm::atan((double)(2.f * Q->x * Q->w + 2.f * Q->y * Q->z), (double)(1 - 2.f * (sqz + sqw)));
	pitchYawRoll.y = -(float)glm::asin((double)(2.f * (Q->x * Q->z - Q->w * Q->y)));
	pitchYawRoll.z = +(float)glm::atan((double)(2.f * Q->x * Q->y + 2.f * Q->z * Q->w), (double)(1 - 2.f * (sqy + sqz)));
